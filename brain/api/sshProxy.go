...
func SshRegister(ctx *gin.Context) {
	...
	if len(name) == 0 || len(username) == 0 {
		ctx.JSON(http.StatusBadRequest, protocols.ProxyMsg{
			Code: -1,
			Msg:  "ERR",
	...
	if err != nil {
		ctx.JSON(http.StatusNotFound, protocols.ProxyMsg{
			Code: -1,
			Msg:  "ERR",
...
func proxyCmd(ctx *gin.Context, name string, cmdType int) bool {
	if name == "brain" {
		...
		ctx.JSON(http.StatusOK, protocols.ProxyMsg{
			Code: 0,
			Msg:  "OK",
	...
	if err != nil {
		...
		ctx.JSON(http.StatusInternalServerError, protocols.ProxyMsg{
			Code: -1,
			Msg:  "ERR",
	...
	if err != nil {
		logger.Comm.Println("protocols.ProxyMsg Unmarshal", err)
		ctx.JSON(http.StatusInternalServerError, protocols.ProxyMsg{
			Code: -1,
			Msg:  "ERR",
			Data: fmt.Sprintf("protocols.ProxyMsg Unmarshal error: %s", err),
		})
		return false
	}
	if pmsg.Code == 0 {
