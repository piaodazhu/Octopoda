...
int main() {
	...
	octl_node_status nstatus[2];
	int statuslen = 2;
	elen = EBUF_LEN;
	ret = octl_get_node_status(targets, 2, nstatus, &statuslen, ebuf, &elen);
	if (ret > 0) {
	...
	for (i = 0; i < statuslen; i++) {
		printf("node %s platform=%s cpu=%f mem=%lld\n", nstatus[i].Name, nstatus[i].Platform, nstatus[i].CpuLoadShort, nstatus[i].MemUsed);
	}
	octl_clear_node_status_list(nstatus, 2);

	octl_brain_info binfo;
	octl_node_info all[32];
	int total_num = 32;
	elen = EBUF_LEN;
	ret = octl_get_node_info(NULL, 0, &binfo, all, &total_num, ebuf, &elen);
	if (ret > 0) {
	...
	octl_clear_node_info_list(all, total_num);

	octl_execution_result results[2];
	int total_results = 2;
	elen = EBUF_LEN;
	ret = octl_run_command("uname -a", 0, targets, 2, results, &total_results, ebuf, &elen);
	if (ret > 0) {
		printf("octl_run: %.*s\n", elen, ebuf);
		return 1;
	}
	...
	octl_clear_execution_result_list(results, total_results);

	total_results = 2;
	elen = EBUF_LEN;
	ret = octl_xrun_command("uname -a", 1, 0, targets, 2, results, &total_results, ebuf, &elen);
	if (ret > 0) {
		printf("octl_run: %.*s\n", elen, ebuf);
		return 1;
	}
	...
	rawLen = 1024;
	ret = octl_get_scenario_version("s1", 0, 2, rawBuf, &rawLen, ebuf, &elen);
	if (ret > 0) {
